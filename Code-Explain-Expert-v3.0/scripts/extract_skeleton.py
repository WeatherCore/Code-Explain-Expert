...
用法:
    python extract_skeleton.py --root <项目根目录> [--out skeleton.json] [--path <目标子目录>]
"""
...
def main() -> int:
    ...
    parser.add_argument("--out", default="skeleton.json", help="输出 JSON 路径")
    ...
    print(f"[OK] 骨架已写入 {out}")
    print(f"     语言: {language} | 文件数: {len(scanned)} | 跳过测试: {skipped_tests} | 依赖边: {len(links)}")
    return 0
