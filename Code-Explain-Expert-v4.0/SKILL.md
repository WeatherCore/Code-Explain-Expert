...
- **注释自检**（Step 3 后必跑）：`scripts/verify_annotations.py --git-root <项目根>`（git 仓库）或 `--original <原文件> --annotated <注释后文件>`（非 git 仓库），验证只加注释未改逻辑；FAIL 立即停止并提示用户回滚（只提示 `git checkout -- <files>` / `git restore <files>`，skill 绝不执行 git 写命令）
...
- **Step 3 后**：抽查 2-3 个已注释文件，确认
  ...
  ⑤注释里的实现机制描述能追溯到 skeleton.json 的行号/类名/方法（证据可追溯，对照 `comment-style-guide.md` §3.5）
