...
**Spring Boot 注释落位三约定**（完整示范见 `samples/Mall-Order/`）：
1. **文件总纲**用 `// ====` 块放在类声明上方；原类已有 Javadoc 时放在 Javadoc 之前、原样保留。需要进 Javadoc 工具的类（对外 API、枚举表）可改写为中文 Javadoc，见样例 `OrderStatus`。
2. **方法"一句话职责"**用 `// 方法名（中文名）：...` 块放在成员上方；原英文 Javadoc 原样保留在其下，绝不改写（两个 Javadoc 连排时只有第一个会绑定成员，所以新增意图统一走 `//` 块）。
3. **接口与实现类分工**：接口只注对外契约（做什么/谁来调/失败承诺），实现类注业务细节与"为什么"，两边不重复。
